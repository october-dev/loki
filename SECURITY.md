# Loki Security Policy

This document describes Loki's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/october-dev/loki/security/advisories/new).
Do not open public issues for security vulnerabilities. **Loki does not
...
- Environment details (`loki --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Loki access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Loki's documentation
...
Loki supports two OS-level isolation postures. They address
...
Loki supports this in two ways:

- **The inherited Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Loki's in-process heuristics
...
Loki filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Loki
itself. Bugs in Loki's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Loki does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Loki's own documentation, or reasonable
  operator expectations would predict — including cases where
  Loki has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Loki's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Loki. Bugs in Loki's skill-install
  or plugin-install path that prevent the
...
- Loki includes supply-chain guards for MCP server
